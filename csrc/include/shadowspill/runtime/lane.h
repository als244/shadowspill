 ...
 * the same fields and the same seven counters, so a transport
 ...
 *
 * A lane that completes on its own schedule has to be deliberate. A value
 * wait it enqueues is satisfied only by its own completion path, so a device
 * call on that path can be blocked by the very wait it exists to satisfy --
 * and not only through the stream: a launch that finds its queue full blocks
 * holding the driver's lock until the device makes progress, and every other
 * thread's device call waits for that lock. The practical form: whatever
 * watches for completions makes no device call at all. Any device work a
 * transfer needs is issued by the thread that called into the lane, on the
 * route stream, behind value waits the watcher satisfies with plain stores;
 * where the hardware touches the pool with no stream in between, the route
 * stream stores a word the watcher reads before it posts.
 *
 * That is why there is no entry here for the runtime to poke a lane with: the
 * worker's loop gates every transfer, so work added there is paid at whatever
 * rate the loop happens to turn, and a poll would be paid on every turn while
