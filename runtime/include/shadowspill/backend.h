...
typedef enum ShadowSpillTransferKind {
    SHADOWSPILL_TRANSFER_TO_DEVICE = 0,
    SHADOWSPILL_TRANSFER_TO_HOST = 1,
 ...
 * progress thread. ShadowSpill serializes state transitions, but the backend
...
typedef struct ShadowSpillBackend {
    ...
    int (*allocate_device)(void *context, uint64_t bytes, void **pointer);
    int (*free_device)(void *context, void *pointer);
    int (*allocate_host)(void *context, uint64_t bytes, void **pointer);
    int (*free_host)(void *context, void *pointer);
