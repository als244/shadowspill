...
from shadowspill.pytorch import Runtime, StepProgram, make_step_program
...
def collect_program(
    ...
    *,
    ...
    print(
        ...
        f"tokens={request.tokens_per_microbatch} "
        f"sequence={request.sequence_length} "
        f"accumulation={request.accumulation_steps}",
        flush=True,
    )
    ...
    try:
        ...
        if relocated and runtime is not None:
            try:
                ...
                externalize_case_model(case, runtime=runtime, release_runtime=True)
