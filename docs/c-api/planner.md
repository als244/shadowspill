...
`shadowspill_evaluate_pressurefit_context` runs the complete deterministic
PressureFit policy portfolio for one resolved recomputation selection. It
...
The caller resolves recomputation alternatives and supplies two immutable
dense views of the same selected program:
...
The complete context evaluator borrows the context, options, simulation
topology, residency facts, seed arrays, and identifier arrays only for the
call. On `SHADOWSPILL_PLANNER_OK` or
...
The evaluator creates no worker threads. Concurrent context calls are safe
