...
config = SimulationConfig.single_device(
    ...
    fetch_bandwidth_bytes_per_second=24_000_000_000,
    evict_bandwidth_bytes_per_second=24_000_000_000,
)
...
- Each device has one FIFO FETCH lane and one FIFO EVICT lane. The directions may
  overlap each other and computation.
- EVICT reserves host capacity at transfer start and retains device capacity until
  completion. FETCH reserves device capacity at transfer start.
- A prefetch submitted while the same alias is offloading waits for EVICT
...
non-retained host extent is released after FETCH completes.
