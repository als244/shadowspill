...
timeline are named `compute_*` and `stream_*`; fields on the host clock sit
under `host` in the records and carry `before_task`, `after_task`,
`dispatch`, `queued`, `reserved`, `dispatched`, or `completion_observed` in
their names. Simulated times are the simulator's own clock shifted so that the
...
`timelines.first_task_start_seconds`, is therefore read here once rather than
...
| `startup_wait_seconds` | Host time at the call's start waiting for the previous invocation to drain. |
...
records' `frontend_lead_seconds` says where.
...
| Simulated | `simulated_ready_seconds`, `simulated_start_seconds`, `simulated_end_seconds`, `expected_profile_seconds` | When the simulator had the inputs ready, when it started and ended the task, and the isolated profile it ran for, which is what the plan was built from. |
| Stream | `compute_reached_seconds`, `compute_started_seconds`, `compute_finished_seconds`, `compute_duration_seconds` | When the compute stream reached the task's readiness marker, when its kernels started after the waits, when they finished, and how long they ran. |
| Stream waits | `input_readiness_wait_seconds`, `allocation_reuse_wait_seconds` | Between reaching and starting: inputs still being fetched, then ranges the task's allocations reuse still owned by a transfer. |
| Delta | `start_delta_seconds`, `end_delta_seconds`, `duration_delta_seconds` | Device minus simulated after alignment for start and end; measured duration minus the profile. |
| Host boundaries | `before_task_enter_seconds`, `before_task_exit_seconds`, `after_task_enter_seconds`, `after_task_exit_seconds`, `frontend_lead_seconds` | When the frontend entered and left each boundary; and how long after the handoff the stream reached the task -- positive is the frontend's lead, zero means the stream caught up. |
| Host costs | `dispatch_before_task_seconds`, `dispatch_invoke_seconds`, `dispatch_after_task_seconds` | The complete `before_task` wall time, the callable's own dispatch, and the complete `after_task` wall time. |
| Inside `before_task` | `dispatch_input_lookup_seconds`, `dispatch_storage_rebind_seconds`, `dispatch_argument_assembly_seconds` | Resolve frontend bindings; rebind changed PyTorch storages; assemble predecoded arguments. Disjoint parts of `dispatch_before_task_seconds`. |
| Inside `after_task` | `dispatch_output_flatten_seconds`, `dispatch_output_classification_seconds`, `dispatch_output_adoption_seconds`, `dispatch_output_state_publish_seconds`, `dispatch_output_publish_seconds`, `dispatch_dematerialize_seconds`, `dispatch_cleanup_seconds` | Flatten the output pytree; match leaves with contracts; adopt returned allocations; publish output state and bindings; publish public outputs; drop released bindings; remove terminal state. Disjoint parts of `dispatch_after_task_seconds`. |

The device timeline of one task is:

```text
stream:   reached ---- input readiness wait ---- allocation reuse wait ---- started ==== kernels ==== finished
host:     before_task enter ---- lookup / rebind / assemble ---- exit    invoke    after_task enter ---- outputs ---- exit
```

A field naming an event -- `reached`, `started`, `finished`, `enter`,
`exit` -- is when that happened; a field naming a span -- `wait`,
`duration`, `lead`, `dispatch` -- is how long it lasted.
...
| Start delta grows along the lane while duration deltas stay small | Readiness, dispatch gaps, or drift on a transfer lane -- read the lanes next. |
...
| A large `before_task` cost | Lookup, rebinding, runtime work, or a capacity wait on the host. |
...
| Simulated | `simulated_ready_seconds`, `simulated_start_seconds`, `simulated_end_seconds`, `simulated_duration_seconds` | When the transfer could start, when the lane started it, when it ended, and its priced duration at the bandwidth the plan assumed, which the plan summary states. `None` for an opening transfer, which the simulator does not model. |
| Stream | `stream_start_seconds`, `stream_end_seconds`, `stream_duration_seconds` | The copy's interval on the lane, bracketed by timing events the worker recorded immediately before and after the copy. `None` when the trace could not measure this transfer. |
| Delta | `start_delta_seconds`, `end_delta_seconds`, `duration_delta_seconds` | Device minus simulated after alignment; `None` without a stream interval or without a simulation. |
| Host | `queued_seconds`, `reserved_seconds`, `dispatched_seconds`, `completion_observed_seconds` | When the action was queued, when its destination was reserved, when the worker handed the copy to the lane, and when the worker's nonblocking poll saw it complete. |

Read `dispatched_seconds` against `stream_start_seconds` to see how long a
copy sat behind its predecessors on the lane, and `completion_observed_seconds`
against `stream_end_seconds` to see the worker's polling lag. Neither gap is
transfer time; the stream interval is.
...
transfers by `sequence` -- and whose `summary` is a `LaneSummary`. `first_task_start_seconds` is where the
first selected task's simulated start fell on the device timeline.

```python
timelines = diagnostics.timelines
print("first task on the device at", timelines.first_task_start_seconds)
for task_id in timelines.compute:
    record = diagnostics.tasks[task_id]
    print(task_id, record.start_delta_seconds, record.duration_delta_seconds)
for lane, records in (
...
| `measured_transfers`, `stream_busy_seconds` | How many transfers carry a stream interval, and the lane time those intervals add up to. |
| `effective_bandwidth_bytes_per_second` | Measured bytes over `stream_busy_seconds`; compare with the assumed bandwidth in the plan summary. `None` when nothing was measured. |
...
| `step_id`, `begin_timestamp_ns`, `end_timestamp_ns` | The trace's identity and its host-clock bounds; the beginning is the host origin the timelines count from. |
