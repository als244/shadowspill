...
class CompiledOutputView:
    """Physical binding for one object leaf returned by the executable."""
...
class CompiledTaskLayout:
    ...
    def additional_workspace_for_outputs(
        ...
        workspace.  Some returned objects are nevertheless transient during
        ...
        for leaf_index in leaf_indices:
            try:
                ...
                raise CaptureError(
                    f"compiled layout has no output leaf {leaf_index}"
                ) from exc
...
def _validate_fresh_bindings(
    ...
    if donated:
        raise CaptureError(
            ...
            "task input; the offline compiler contract is incomplete"
        )
...
def _resolve_fresh_allocation(
    ...
    if event.requested_bytes != requested:
        raise CaptureError(
            "allocator profile disagrees with the compiler's output extent: "
