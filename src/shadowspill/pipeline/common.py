"""What every planning phase shares: its clock, its budgets, its refusals.

The timer a phase measures itself with, the budget and capacity arithmetic the
search is configured from, and the two public errors a refused plan raises.
Framework-neutral; the two checks that need a framework are the frontend's.
"""
...
from shadowspill.task.profiles import ProfilingResult, TaskMeasurement
...
def build_simulation_config(
    ...
    *,
    ...
    """Build framework-neutral simulator input for one ShadowSpillProgram.

    ``execution_device_id`` is the IR identity of the device the plan runs on;
    only a frontend can name it, so it is passed in. The calibration comes from
    `planned_transfer_bandwidths`, so what the simulator is built with is what
    any caller reporting this run's lanes
