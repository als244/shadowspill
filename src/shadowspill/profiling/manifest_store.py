...
the compiler storage contract from that same compilation, allowing planning to
...
from shadowspill.task.profiles import ProfileKey
from shadowspill.task.storage import TaskStorageContract
...
class CompiledManifestStore:
    ...
    def __init__(
        ...
        *,
        ...
        #: What compiled the manifests this store caches. A manifest is only
        #: valid for the toolchain that produced it, and only a frontend knows
        #: what that is, so the store is told once rather than asking.
        self._toolchain = dict(toolchain)
