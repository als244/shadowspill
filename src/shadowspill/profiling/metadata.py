...
def repeated_profiling_metadata(
    value: Sequence[object] | None,
    *,
    repetitions: int,
) -> tuple[ProfilingMetadata, ...]:
    if value is None:
        return tuple(canonicalize_profiling_metadata(None) for _ in range(repetitions))
    if isinstance(value, str | bytes) or not isinstance(value, Sequence):
        raise TypeError("profiling_metadata must be a sequence or None")
    if len(value) != repetitions:
        raise ValueError("profiling_metadata must have one entry per repetition")
