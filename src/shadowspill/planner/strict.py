"""Strict readers for one JSON field at a time.

Every half of the planner that reads a record back uses these, so one
malformed record refuses in the same words whichever half parsed it. A leaf
module on purpose: it imports nothing from the package around it, which is what
lets the serialization, diagnostics and admission readers all share it without
