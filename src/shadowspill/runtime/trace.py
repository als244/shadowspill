...
def begin_runtime_trace(
    runtime_handle: int, *, step_id: int, origin: Marker | None = None
) -> None:
    """Begin the prepared trace, measuring transfers from ``origin``.

    ``origin`` is the caller's marker, already recorded on the compute stream,
    so the trace and the caller share one timeline. It must outlive the trace.
    ``None`` records no stream intervals.
    """

    status = int(
        runtime_library().shadowspill_trace_begin(
            runtime_handle, step_id, origin._handle if origin is not None else None
        )
    )
    if status != 0:
